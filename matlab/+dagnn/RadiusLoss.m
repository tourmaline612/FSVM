classdef RadiusLoss < dagnn.ElementWise
  properties
    loss = 'softmaxlog'
    ignoreAverage = false
    opts = {}
  ...
  methods
    function outputs = forward(obj, inputs, params)
      outputs{1} = vl_nnradiusloss(inputs{1}, inputs{2}, inputs{3}) ;
    ...
    function [derInputs, derParams] = backward(obj, inputs, params, derOutputs)
      derInputs{1} = vl_nnradiusloss(inputs{1}, inputs{2},  inputs{3}, derOutputs{1}) ;
      derInputs{2} = [] ;
      derInputs{3} = [] ;
      derParams = {} ;
    end
    ...
    function obj = RadiusLoss(varargin)
      obj.load(varargin) ;
    end
  end
end
