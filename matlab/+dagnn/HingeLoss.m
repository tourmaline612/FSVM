classdef HingeLoss < dagnn.ElementWise
  properties
    loss = 'softmaxlog'
    ignoreAverage = false
    opts = {}
  ...
  methods
    function outputs = forward(obj, inputs, params)
      outputs{1} = vl_nnhingeloss(inputs{1}, inputs{2}) ;
    ...
    function [derInputs, derParams] = backward(obj, inputs, params, derOutputs)
      derInputs{1} = vl_nnhingeloss(inputs{1}, inputs{2}, derOutputs{1}) ;
      derInputs{2} = [] ;
      derParams = {} ;
    end
    ...
    function obj = HingeLoss(varargin)
      obj.load(varargin) ;
    end
  end
end
