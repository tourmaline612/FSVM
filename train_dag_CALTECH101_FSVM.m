function [net,stats] = train_dag_CALTECH101_FSVM(net, imdb, getBatch, varargin)
...
if start >= 1
  ...
  restart_center_data = gather(state.center_data);
else
  state = [] ;
end
...
if isfield(params,'restart_center_data')
    if ~isempty(params.restart_center_data)
        if numel(params.gpus) >= 1
            restart_center_data = gpuArray(params.restart_center_data);
        end
    end
end
...
for t=1:params.batchSize:numel(subset)
  ...
  for s=1:params.numSubBatches
    ...
    nClass = 102;    
    if epoch==1 && t==1,
        dim = numel(net.params(net.getParamIndex('fc_b')).value);
        center_data = single(zeros(dim ,nClass));
    ...
    if strcmp(mode, 'train')
      ...
      data = squeeze(gather(net.vars(net.getVarIndex('fc')).value));
      labels = inputs{4};
      alpha = 0.5;
      for c =1 : nClass
          index = find( labels == c );
      ...
      fprintf(' Data: %f,  Delta: %f  ', norm(data(:,1)), norm(alpha * delta_c(:,1)));
...
sel = find(cellfun(@(x) isa(x,'dagnn.CenterLoss'), {net.layers.block})) ;
for i = 1:numel(sel)
  if net.layers(sel(i)).block.ignoreAverage, continue; end;
  stats.(net.layers(sel(i)).outputs{1}) = net.layers(sel(i)).block.average ;
end
