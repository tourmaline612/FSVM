function [ model, L ] = FSVM_train_update_Sw(Ytrain, trainset, option, opts, rho)
% By using the Sw (the intra-class scatter matrix), updating Sw
% trainset: nxd
% 
...
while stop > threshold && iter <= maxiter
    ...
    % update the intra-class scatter matrix
    S = update_Sw(coef, trainset, Ztrain, Ytrain);
    t = beta * t;
    obj_previous = obj;
    iter = iter+1;
end
