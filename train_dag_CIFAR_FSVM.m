function [net,stats] = train_dag_CIFAR_FSVM(net, imdb, nClasses, getBatch, varargin)
...
% chose which loss for BP
opts.derOutputs = {'Lw_obj', 1, 'LM_obj', 1} ;
...
if start >= 1
  ...
  restart_center_data = gather(state.center_data); %also reload the center_data!
else
  state = [] ;
end
...
for epoch=start+1:opts.numEpochs
  ...
  params.start = start;
  if start >= 1
      params.restart_center_data = restart_center_data;
  end
...
if isfield(params,'restart_center_data')
    if ~isempty(params.restart_center_data)
        if numel(params.gpus) >= 1
            center_data = gpuArray(params.restart_center_data);
        end
    end
end
...
for t=1:params.batchSize:numel(subset)
  ...
  for s=1:params.numSubBatches
    ...
    if epoch==1 && t==1,
          dim = numel(net.params(net.getParamIndex('transform_b')).value);
          center_data = single(zeros(dim ,nClasses));
    elseif params.start>=1 && (params.start +1)==epoch
        center_data = restart_center_data;
    else
        center_data = state.center_data;
    end
    
    n_inputs = numel(inputs); 
    inputs{n_inputs+1} = 'center_data'; 
    inputs{n_inputs+2} = center_data;
    ...
    if strcmp(mode, 'train')
      ...
      % update the center_data
      data = squeeze(net.vars(net.getVarIndex('transform_bn')).value);
      labels = inputs{4};
      alpha = 0.5; %adjusting the alpha may lead to better result
      for c =1 : nClasses
          index = find( labels == c );
          if numel(index) < 1
               continue,
          end
          diff = - bsxfun(@minus, data(:,index), center_data(:,c));  % c - x
          numerator = sum( diff, 2); 
          denominator = 1 + numel(index);
          delta_c = numerator / denominator;
          center_data(:, c) = gather(center_data(:,c)) - gather(alpha * delta_c);
      end      
      idx = net.getVarIndex('center_data');
      if isa(inputs{2},'gpuArray')
          center_data = gpuArray(center_data);
      end
      net.vars(idx).value = center_data;
      net.backward(params.derOutputs, 'holdOn', s < params.numSubBatches) ;
    else
      net.mode = 'test' ;
      net.eval(inputs) ;
    end
...
sel = find(cellfun(@(x) isa(x,'dagnn.HingeLoss'), {net.layers.block})) ;
for i = 1:numel(sel)
  if net.layers(sel(i)).block.ignoreAverage, continue; end;
  stats.(net.layers(sel(i)).outputs{1}) = net.layers(sel(i)).block.average ;
end
sel = find(cellfun(@(x) isa(x,'dagnn.RadiusLoss'), {net.layers.block})) ;
for i = 1:numel(sel)
  if net.layers(sel(i)).block.ignoreAverage, continue; end;
  stats.(net.layers(sel(i)).outputs{1}) = net.layers(sel(i)).block.average ;
end
